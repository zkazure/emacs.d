...
;; - Reuse view-mode keybindings
;; - Automatically enable for buffers that are not already in view-mode

(require 'view)

(defvar view-only-mode-map
  (let ((map (make-sparse-keymap)))
    ;; Inherit all view-mode bindings
    (set-keymap-parent map view-mode-map)
    map))

(define-minor-mode view-only-mode
  "A view-only state."
  :lighter " ViewOnly"
  :keymap view-only-mode-map
...
(defun my-enable-view-only-mode ()
  "Enable view-only-mode unless view-mode is already active."
  (when (and global-view-only-mode
             (not view-only-mode))
    (view-only-mode 1)))


;; Global controller
(define-minor-mode global-view-only-mode
  "Enable view-only-mode globally for new and existing buffers."
  :global t
  :lighter " GlobalViewOnly"

  (if global-view-only-mode
      (progn
        ;; Enable for existing buffers
        (dolist (buffer (buffer-list))
          (with-current-buffer buffer
            (my-enable-view-only-mode)))

        ;; Enable for future buffers
